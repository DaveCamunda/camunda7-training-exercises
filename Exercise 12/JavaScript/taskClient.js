...
client.subscribe("creditCardCharging", async function({ task, taskService }) {
    ...
    if (isInvalidExpiryDate(expiryDate)) {
        ...
        console.log("Charge error: Date is invalid");

        await taskService.handleBpmnError(task, "creditCardChargeError", "Date is invalid", null);
...
client.subscribe("creditRestore", async function({ task, taskService }) {

    console.log("Restored customer's credit: " + task.variables.get("customerCredit") + " EUR");
